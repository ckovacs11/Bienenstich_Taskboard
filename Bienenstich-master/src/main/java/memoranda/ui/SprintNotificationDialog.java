...
public class SprintNotificationDialog extends JFrame {
  ...
  public SprintNotificationDialog(String title, String time, String text) {
    super();
    this.setTitle(title);
    try {
      jbInit();
  ...
  public SprintNotificationDialog() {
    this("", "", "");
  }
  void jbInit() throws Exception {
    this.setResizable(false);
    this.setIconImage(new ImageIcon(SprintNotificationDialog.class.getResource("/ui/icons/jnotes16.png")).getImage());
  ...
  private void playSoundNotification() {
		...
		if (Configuration.get("NOTIFY_SOUND").equals("DEFAULT"))
			url =
				SprintNotificationDialog.class.getResource(
					"/ui/beep.wav");
		else
			try {
				url =
