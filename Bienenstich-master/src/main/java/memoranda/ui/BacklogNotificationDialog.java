...
public class BacklogNotificationDialog extends JFrame {
  ...
  public BacklogNotificationDialog(String title, String time, String text) {
    super();
    this.setTitle(title);
    try {
      jbInit();
    ...
    timeLabel.setIcon(new ImageIcon(TaskDialog.class.getResource(
  ...
  public BacklogNotificationDialog() {
    this("", "", "");
  }
  void jbInit() throws Exception {
    this.setResizable(false);
    this.setIconImage(new ImageIcon(BacklogNotificationDialog.class.getResource("/ui/icons/jnotes16.png")).getImage());
  ...
  private void playSoundNotification() {
		...
		if (Configuration.get("NOTIFY_SOUND").equals("DEFAULT"))
			url =
				BacklogNotificationDialog.class.getResource(
					"/ui/beep.wav");
		else
			try {
				url =
