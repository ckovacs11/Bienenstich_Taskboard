...
*/
	...
	static {
		//CurrentStorage.get().openSprintsManager();
		if (_doc == null) {
			_root = new Element("sprintslist");
/*			_root.addNamespaceDeclaration("jnsprints", NS_JNSPRINTS);
			_root.appendChild(
	...
	public static ArrayList<Sprint> getSprints(){
		/*for(int i = 0; i < sprints.size(); i++){
			System.out.println("in getsprints " + sprints.get(i).getName());
		}*/
		return sprints;
	}

	public static Element createSprint(CalendarDate startDate, CalendarDate endDate, String name) {
	...
	//TODO
	public static Collection getActiveSprints() {

		Collection activeSprints = null;

		for (int i = 0; i < sprints.size(); i++){
			if(CalendarDate.today().inPeriod(sprints.get(i).getStartDate(), sprints.get(i).getEndDate())) {
				//sprint is active add too collection
				activeSprints.add(sprints.get(i));
			}
		}
	...
	//todo returns all sprints, doesnt filter by active attribute. need to implement this
	...
	public static Sprint getSprint(CalendarDate date, int hh, int mm) {
		Day d = getDay(date);
		if (d == null)
			return null;
		Elements els = d.getElement().getChildElements("sprint");
		for (int i = 0; i < els.size(); i++) {
			...
			if ((new Integer(el.getAttribute("hour").getValue()).intValue()
				...
				&& (new Integer(el.getAttribute("min").getValue()).intValue()
					== mm))
				return new SprintImpl(el);
		}
		return null;
	}

	public static void removeSprint(Sprint spr) {
